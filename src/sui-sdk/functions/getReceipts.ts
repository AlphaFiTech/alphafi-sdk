...
// export async function getMultiBluefinPool() {
//   let pools = Object.keys(bluefinPoolMap);
//   pools = pools.filter((pool) => {
//     return bluefinPoolMap[pool] !== "";
//   });
//   const poolIds = pools.map((pool) => {
//     return bluefinPoolMap[pool];
//   });
...
//     batches.push(batchEntries); // Convert to object before pushing
//   }
//   for (const bluefinPoolIdsBatch of batches) {
//     try {
//       const o = await getSuiClient().multiGetObjects({
//         ids: bluefinPoolIdsBatch,
//         options: {
//           showContent: true,
//         },
//       });
//       for (let i = 0; i < pools.length; i = i + 1) {
//         const poolData = o[i].data as BluefinPoolType;
//         const cacheKey = `bluefinPool_${bluefinPoolMap[pools[i]]}`;
//         cetusPoolCache.set(cacheKey, poolData);
//       }
//     } catch (e) {
//       console.error(`Error getting multiBluefinPool`, e);
//     }
//   }
// }

// export async function getBluefinPool(
//   poolName: string,
//   ignoreCache: boolean,
// ): Promise<CetusPoolType | BluefinPoolType | undefined> {
//   const suiClient = getSuiClient();
//   const cacheKey = `bluefinPool_${bluefinPoolMap[poolName]}`;
//   if (ignoreCache) {
//     cetusPoolCache.delete(cacheKey);
//     cetusPoolPromiseCache.delete(cacheKey);
//   }
...
//     try {
//       const o = await suiClient.getObject({
//         id: bluefinPoolMap[poolName],
//         options: {
//           showContent: true,
//         },
//       });

//       const bluefinPool = o.data as BluefinPoolType;

//       // Cache the pool object
//       cetusPoolCache.set(cacheKey, bluefinPool);
//       return bluefinPool;
//     } catch (e) {
//       console.error(`getBluefinPool failed for poolName: ${poolName}`);
//       return undefined;
//     } finally {
//       // Remove the promise from the cache after it resolves
//       cetusPoolPromiseCache.delete(cacheKey);
